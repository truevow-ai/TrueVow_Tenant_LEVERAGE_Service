-- ============================================================================
-- CITATION VERIFICATION REMEDIATION SCRIPT v2
-- ============================================================================
-- Purpose: Update county-level citations with VERIFIED excerpts
-- Approach: Keep citation names, update content only
...
-- ============================================================================
-- CALIFORNIA COUNTY CORRECTIONS - Update excerpts only
-- ============================================================================
...
-- Montgomery County - VERIFIED from montcourt.oh.gov
UPDATE leverage.rule_citations 
SET 
    source_url = 'https://www.montcourt.oh.gov/wp-content/uploads/2025/01/New-Rules-Final-Doc16-Updated-12-30-24-eff.-1-1-25.pdf',
    excerpt = 'Local Rule 1.15 - Electronic Filing. Electronic filing is required for all civil cases. Proposed orders must be submitted in PDF format. Default method of service by the Clerk is FedEx. Incorporates waiver of service provisions.',
    verifier = 'web_verified_official',
    last_verified_at = NOW()
FROM leverage.legal_sources ls
WHERE rule_citations.legal_source_id = ls.id
...
-- ============================================================================
-- MARK REMAINING UNVERIFIED CITATIONS
-- ============================================================================

-- Mark citations not yet verified as needing review
UPDATE leverage.rule_citations 
SET 
    confidence_level = 'medium',
    verifier = 'ai_approximated_needs_review',
    notes = 'Citation requires verification by legal professional. Content approximated from court website patterns.'
...
AND verifier != 'web_verified_official';
...
ORDER BY 
    CASE WHEN rc.verifier = 'web_verified_official' THEN 0 ELSE 1 END,
    ls.jurisdiction_state, 
    ls.jurisdiction_county;
