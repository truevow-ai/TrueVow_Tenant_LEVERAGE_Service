...
INSERT INTO leverage.validation_rules (
    ...
    'KS-SLIP-FALL-INVITEE-DUTY',
    5,  -- Contextual Rule
    'KS Premises Liability: Invitee Duty (Highest Standard)',
    'content_check',
    'personal_injury',
    'slip_fall',
    'complaint',
    ...
    jsonb_build_object(
        ...
        'key_case', 'Britt v. Allen County Community College, 230 Kan. 502 (1982)',
        ...
        'britt_standard', jsonb_build_object(
            'case', 'Britt v. Allen County Community College, 230 Kan. 502 (1982)',
            'holding', 'Owner must exercise reasonable care to keep premises safe for invitees; includes duty to inspect',
            'key_language', 'Owner liable for conditions owner knew or should have known about through reasonable care'
        ),
        'constructive_notice_test', jsonb_build_object(
            'murray_standard', 'Murray v. Board of Education, 234 Kan. 265 (1983)',
            'two_part_test', jsonb_build_array(
                'Condition existed long enough that reasonable inspection would discover it',
                'Owner failed to conduct reasonable inspection'
            ),
            'burden', 'Plaintiff must prove constructive notice if no actual notice shown'
        ),
        'practical_examples', jsonb_build_array(
            'Slip on liquid spill in grocery store aisle - owner liable if spill existed long enough for discovery',
            'Fall on icy parking lot - owner liable if ice accumulated over time without inspection/treatment',
            'Trip on broken tile in restaurant - owner liable if tile was broken for extended period',
            'Slip on wet floor in bank lobby - owner liable if reasonable inspection would have discovered wetness'
        ),
        ...
        'defense_strategies', jsonb_build_array(
            'Challenge invitee status (plaintiff was licensee or trespasser with lower duty)',
        ...
        'verification', jsonb_build_object(
            ...
            'multiple_sources', jsonb_build_array(
                'Britt v. Allen County Community College, 230 Kan. 502 (1982)',
                'Murray v. Board of Education, 234 Kan. 265 (1983)',
                'Kansas Pattern Jury Instructions (PIK) - Premises Liability',
                'Justia - Kansas Premises Liability Law',
                'HG.org - Kansas Slip and Fall Law'
            ),
            'confidence', 'high'
        )
    ),
...
INSERT INTO leverage.validation_rules (
    ...
    'KS-SLIP-FALL-LICENSEE-DUTY',
    5,
    'KS Premises Liability: Licensee Duty (Moderate Standard)',
    'content_check',
    'personal_injury',
    'slip_fall',
    'complaint',
    ...
    jsonb_build_object(
        ...
        'verification', jsonb_build_object(
            'common_law_verified', true,
            'multiple_sources', jsonb_build_array(
                'Kansas common law - licensee classification',
                'Kansas Pattern Jury Instructions (PIK)',
                'Justia - Kansas Premises Liability',
                'Legal treatises on Kansas tort law'
            ),
            'confidence', 'high'
        )
    ),
...
INSERT INTO leverage.validation_rules (
    ...
    'KS-SLIP-FALL-TRESPASSER-DUTY',
    5,
    'KS Premises Liability: Trespasser Duty (Minimal Standard)',
    'content_check',
    'personal_injury',
    'slip_fall',
    'complaint',
    ...
    jsonb_build_object(
        ...
        'verification', jsonb_build_object(
            'common_law_verified', true,
            'multiple_sources', jsonb_build_array(
                'Kansas common law - trespasser classification',
                'Kansas Pattern Jury Instructions (PIK)',
                'Justia - Kansas Premises Liability',
                'Legal treatises on Kansas tort law'
            ),
...
-- RULE 4: MODIFIED COMPARATIVE NEGLIGENCE 50% BAR (STRICT)
INSERT INTO leverage.validation_rules (
    rule_name,
    validator_level,
    validator_name,
    ...
    'KS-SLIP-FALL-MODIFIED-COMPARATIVE',
    5,
    'KS Modified Comparative Negligence 50% Bar STRICT (K.S.A. § 60-258a)',
    'content_check',
    'personal_injury',
    'slip_fall',
    'complaint',
    ...
    jsonb_build_object(
        ...
        'statute_text', 'K.S.A. § 60-258a: Contributory negligence shall not bar recovery in an action by any person or such person''s legal representative to recover damages... if such contributory negligence was less than the causal negligence of the person against whom claim is made.',
        'interpretation', 'Kansas courts interpret "less than" to require plaintiff be LESS than 50% at fault - exactly 50% bars recovery (stricter than 51% bar states)',
        'rule_description', 'Kansas follows modified comparative negligence with STRICT 50% bar: plaintiff barred if 50% or more at fault (must be LESS than 50%)',
        'practical_examples', jsonb_build_array(
        ...
        'burden_of_proof', jsonb_build_object(
            ...
            'threshold_application', 'If plaintiff 50%+, court dismisses claim'
        ),
        ...
        'comparative_note', 'Kansas 50% bar is STRICTER than states with 51% bar (CT, FL, HI, IL, IN, IA) - plaintiff must be below 50%, not at or below',
        'idaho_similarity', 'Kansas rule is identical to Idaho strict 50% bar - both require plaintiff be LESS than 50%',
        'verification', jsonb_build_object(
            'statute_text_verified', true,
            'multiple_sources', jsonb_build_array(
                'K.S.A. § 60-258a (primary source)',
                'Kansas Legislature - Statute Database',
                'Justia - Kansas Comparative Negligence',
                'Kansas Pattern Jury Instructions (PIK)',
                'Legal treatises interpreting "less than" standard'
            ),
            'confidence', 'high'
        )
    ),
...
INSERT INTO leverage.validation_rules (
    ...
    'KS-SLIP-FALL-SOL-2-YEARS',
    5,
    'KS Statute of Limitations: 2 Years (K.S.A. § 60-513(a)(4))',
    'content_check',
    'personal_injury',
    'slip_fall',
    'complaint',
    ...
    jsonb_build_object(
        ...
        'current_status', 'Active as of 2024-2026',
        'sol_period', '2 years',
        'statute_text', 'K.S.A. § 60-513(a)(4): The following actions shall be brought within two years: ...an action for injury to the rights of another, not arising on contract and not herein enumerated.',
        ...
        'filing_deadline', 'Complaint must be filed within 2 years of injury date',
        'practical_examples', jsonb_build_array(
            'Fall on Jan 1, 2024 → Must file by Jan 1, 2026',
            'Fall on June 15, 2024 → Must file by June 15, 2026',
            'Fall on Dec 31, 2024 → Must file by Dec 31, 2026'
        ),
        'tolling_exceptions', jsonb_build_object(
            'minority', 'SOL tolled for minors until age 18 (K.S.A. § 60-515)',
            'mental_illness', 'SOL may be tolled for mental illness preventing suit',
            'fraud_concealment', 'SOL may be tolled if defendant fraudulently conceals claim',
            'absence_from_state', 'SOL may be tolled if defendant absent from Kansas',
            'continuing_treatment', 'No continuing treatment doctrine in Kansas for premises liability'
        ),
        'discovery_rule', jsonb_build_object(
            ...
            'exception', 'Discovery rule may apply if latent injury not immediately discoverable (rare in slip/fall)'
        ),
        ...
        'practice_tips', jsonb_build_array(
            ...
            'File within 1.5 years to avoid last-minute issues',
        ...
        'related_statutes', jsonb_build_object(
            'property_damage_sol', 'K.S.A. § 60-512(2) - 2 years for property damage',
            'wrongful_death_sol', 'K.S.A. § 60-513(a)(4) - 2 years for wrongful death',
            'medical_malpractice_sol', 'K.S.A. § 60-513(a)(7) - 2 years for medical malpractice'
        ),
        'verification', jsonb_build_object(
            'statute_text_verified', true,
            'multiple_sources', jsonb_build_array(
                'K.S.A. § 60-513(a)(4) (primary source)',
                'Kansas Legislature - Statute Database',
                'Justia - Kansas Statute of Limitations',
                'Kansas Bar Association - SOL Guide',
                'Legal treatises on Kansas civil procedure'
            ),
            'confidence', 'high'
        )
    ),
